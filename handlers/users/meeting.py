...
from enum_constans import meeting_status_constant, waiting_status_constant, not_ready_status_constant, see_you_in_week, \
    find_companion_on_Monday, error_contact_support, change_meeting_status_for_new_profile
...
# Используется, когда новый пользователь заполнил skills и нажал "сотворить магию"
@dp.callback_query_handler(meeting_status_callback.filter(status="meeting_status = do magic"))
async def meeting_status_do_magic(callback: CallbackQuery):
    await callback.answer(cache_time=10)

    token = login(callback.from_user.id, constants.machine_token_constant).json().get("token")

    if patch(waiting_status_constant, token) == 200:
        await callback.message.answer(change_meeting_status_for_new_profile)
